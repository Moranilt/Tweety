<div class="flex">
    ...
    <form id="like-tweet-form" action="/tweets/{{ $tweet->id }}/like" method="POST">
        @csrf
        <div id="like-content" style="cursor:pointer"
            class=" like-content flex items-center mr-4 {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
            ...
            <i class="{{ $tweet->isLikedBy(current_user()) ? 'fas' : 'far' }} fa-heart mr-1"></i>
            <input type="hidden" name="like-tweet-id" value="{{ $tweet->id }}" class="like-tweet-id">

            <button type="submit" class="like-tweet text-xs font-bold">

                {{ $tweet->likes()->where('liked', '1')->count() ?: 0 }}
            </button>
        </div>
    </form>
   <!-- <form action="/tweets/{{ $tweet->id }}/like" method="POST">
        ...
        @method('DELETE')
        <div
            class="{{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }} flex items-center">
            <svg viewBox="0 0 20 20"
                 class="{{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }} mr-1 w-3">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
                        <path
